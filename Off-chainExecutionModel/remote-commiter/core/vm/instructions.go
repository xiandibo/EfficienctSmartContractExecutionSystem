...
func opCall(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	//log.Println("====== Internal Call ====== ")
	//log.Info("调用远程节点的合约\t", "节点地址:", addrtoip.AddressToIP[toAddr], "\t", "合约地址:", toAddr)
	//scpeer, _ := clientpeer.NewFollowerClientPeer(addrtoip.AddressToIP[toAddr], nil)
	//fmt.Println(toAddr)
	//fmt.Println(addrtoip.AddressToIP[toAddr])
	//fmt.Println("PC:", *pc)
	conn := addrtoip.NodeAddrConnMap[addrtoip.AddressToIP[toAddr]]
	scpeer := &clientpeer.FollowerClient{Connection: pb.NewFollowerClient(conn)}
	res, err1 := scpeer.InternalCallSmartContract(context.Background(),&pb.InputForSC{Address: toAddr.Bytes(), Input: args, Txid: interpreter.evm.TxID})
	//log.Println("args ====== ", args)
	//log.Println("Internal Call ====== ", res.Result)
	//log.Info(res)
	if err1 != nil {
		fmt.Println(addrtoip.AddressToIP[toAddr])
		fmt.Println(err1)
	}
	ret, returnGas := res.Result, gas
	var err error

	//fmt.Println("SubCall", interpreter.evm.TxID, ret)

	//修改此调用为调用远程节点的方法
	//ret, returnGas, err := interpreter.evm.Call(callContext.contract, toAddr, args, gas, value)
	//log.Println("Internal Call ====== ", ret)
...
func opStaticCall(pc *uint64, interpreter *EVMInterpreter, callContext *callCtx) ([]byte, error) {
	...
	log.Println("Internal StaticCall ====== start")
	ret, returnGas, err := interpreter.evm.StaticCall(callContext.contract, toAddr, args, gas)
	log.Println("Internal StaticCall ====== ", ret)
	if err != nil {
		callContext.stack.push(interpreter.intPool.getZero())
	} else {
		callContext.stack.push(interpreter.intPool.get().SetUint64(1))
